...
export async function main(ns: NS): Promise<void> {
  ...
  while (true) {
    ...
    if (
      ...
      const weakenThreadsRequired = Math.ceil(
        (ns.getServerSecurityLevel(target) -
          ns.getServerMinSecurityLevel(target)) /
          ns.weakenAnalyze(1, coresAvailable)
      );
      const weakenThreadsAvailable = Math.floor(
        (ns.getServerMaxRam("home") - ns.getServerUsedRam("home")) /
          ns.getScriptRam("/batching/weaken.js", "home")
      );
    ...
    // Grow
      ...
      const growThreadsRequired = Math.ceil(
        ns.growthAnalyze(
          target,
          ns.getServerMaxMoney(target) / currentMoney,
          coresAvailable
        )
      );
      const growThreadsAvailable = Math.floor(
        (ns.getServerMaxRam("home") - ns.getServerUsedRam("home")) /
          ns.getScriptRam("/batching/grow.js", "home")
      );
    ...
    // Hack
      ...
      const hackThreadsRequired = Math.ceil(
        ns.hackAnalyzeThreads(target, ns.getServerMaxMoney(target) * 0.75)
      );
      const hackThreadsAvailable = Math.floor(
        (ns.getServerMaxRam("home") - ns.getServerUsedRam("home")) /
          ns.getScriptRam("/batching/hack.js", "home")
      );
