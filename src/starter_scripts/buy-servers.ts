...
export async function main(ns: NS): Promise<void> {
  ...
  const purchasedServersCount = purchasedServers.length;

  // Check how much money the player has.

  const playerMoney = ns.getServerMoneyAvailable("home");
  const formattedPlayerMoney = playerMoney.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  });
  ...
  const maxServers = Math.floor(playerMoney / 25);
  const formattedMaxServers = maxServers.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  });
  ...
  if (mode == "check") {
    ...
    ns.tprint(
      "I already have " + purchasedServersCount + " servers purchased."
    );
    // Print out how much money I have.
    ns.tprint("You have: " + formattedPlayerMoney + " available.");
    // Print max cost of servers.
    ns.tprint(
      "I can purchase 25 servers with a max cost of: " + formattedMaxServers
    );
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    const serverCost = ns.getPurchasedServerCost(Math.pow(2, i));
    // Format serverCost into US Dollars
    const formattedServerCost = serverCost.toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    });
    ...
    if (serverCost < maxServers) {
      ...
      if (mode == "check") {
        ns.tprint(
          "A " + Math.pow(2, i) + "GB server costs: " + formattedServerCost
        );
      }
    }
  }
